...
2. The input is validated: email must be a valid format (`user@domain.tld`), repo must match GitHub naming rules (alphanumeric, hyphens, dots; no special chars or path traversal). Invalid input is rejected **before** any external API call.
...
A **cron job runs every 5 minutes**, checking the GitHub API for new releases on all repositories that have at least one confirmed subscriber. It uses **ETags** for conditional requests — if nothing changed, GitHub returns `304 Not Modified` and no rate limit is consumed. When a new release tag is detected (different from `lastSeenTag`), notification emails are queued in bulk for all subscribers of that repo.
...
This starts PostgreSQL, Redis, and the app. Prisma migrations run automatically on startup. The app is available at `http://localhost:3000`.
